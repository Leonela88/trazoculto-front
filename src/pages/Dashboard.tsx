...
const Dashboard: React.FC = () => {
  ...
  const [editForm, setEditForm] = useState<EditForm>({ projectTitle: "", clientName: "", state: "" });
    ...
    <div className={styles.container}>
      ...
      <div className={styles.sectionHeader}>
        <img
          ...
          onClick={() => {
            // Cycle through filters as a simple implementation for the filter icon
            const states = ["ALL", "PENDING", "IN_PROGRESS", "COMPLETED", "CANCELLED"];
            const nextIndex = (states.indexOf(filterState) + 1) % states.length;
            setFilterState(states[nextIndex]);
          }}
          style={{ cursor: 'pointer' }}
        />
        <Title color="dark">Mis Proyectos ({filterState})</Title>
      </div>
